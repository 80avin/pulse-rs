...
use pulse_core::ai::rules::{RuleEngine, default_rules, evaluate_low_effort};
...
fn test_coverage_on_tagged_items() {
    ...
    let tagged_fixtures: Vec<_> = all_fixtures
        .iter()
        .enumerate()
    ...
    println!(
        "Rule engine coverage: {covered}/{total} = {:.1}%",
        coverage * 100.0
    );
...
fn test_negative_items_get_no_unwanted_tags() {
    ...
    let negatives: Vec<_> = all_fixtures
        .iter()
        .enumerate()
...
fn test_low_effort_only_on_reddit() {
    ...
    assert!(
        tags_reddit.contains(&"low-effort".to_string()),
        "Expected low-effort on Reddit"
    );

    // Must NOT fire on HN or RSS
    let tags_hn = run_rules(&engine, &reddit_item, &FeedType::Hn);
    assert!(
        !tags_hn.contains(&"low-effort".to_string()),
        "low-effort should not fire on HN"
    );

    let tags_rss = run_rules(&engine, &reddit_item, &FeedType::Rss);
    assert!(
        !tags_rss.contains(&"low-effort".to_string()),
        "low-effort should not fire on RSS"
    );
}
...
fn test_rule_engine_is_multi_label() {
    ...
    assert!(
        ...
        "Expected tutorial (from scratch)"
    );
    assert!(
        tags.contains(&"technical".to_string()),
        "Expected technical (rust)"
    );
...
fn test_privacy_false_positive_guard() {
    ...
    let tags = run_rules(&engine, &item, &FeedType::Hn);
    assert!(
        !tags.contains(&"privacy".to_string()),
        "False positive: 'issue tracker' should not trigger privacy tag, got {:?}",
        tags
    );
}
