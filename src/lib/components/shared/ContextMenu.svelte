...
{#if open}
  <Portal>
    {#if mode === 'popup'}
      <div class="fixed inset-0 z-[100] bg-black/40 anim-sheet-overlay-in" role="presentation" aria-hidden="true" style="transform:translateZ(0);will-change:transform;pointer-events:auto;" onclick={onClose}></div>
      <div
        bind:this={menuEl}
        tabindex="-1"
        ...
        style="left:{pos.x}px;top:{pos.y}px;transform:translateZ(0);will-change:transform;pointer-events:auto;"
      >
        {@render children()}
      </div>
    {:else}
      ...
      <div
        ...
        style="padding:8px;padding-bottom:max(8px,env(safe-area-inset-bottom));transform:translateZ(0);will-change:transform;pointer-events:auto;"
      >
        {@render children()}
      </div>
    {/if}
